...
`dotpm` reads and edits the projects and tasks of a vault open in Obsidian with the [dotpm](https://github.com/dotpm/obsidian-pm) plugin, from a terminal, a script or an agent. It talks to the plugin through its [local API](https://github.com/dotpm/obsidian-pm/blob/main/docs/api.md), so that has to be on: Settings > Local API in Obsidian. Everything it changes goes through the same code the views use and shows in Obsidian at once.
...
Anything else, such as `recurrence` or `customFields`, goes in `--data` as a JSON object using the field names from [the API](https://github.com/dotpm/obsidian-pm/blob/main/docs/api.md). `--data -` reads it from stdin, and flags override what it holds.
...
Most MCP clients can use the plugin's HTTP endpoint directly, as described in [api.md](https://github.com/dotpm/obsidian-pm/blob/main/docs/api.md). For a client that only launches a command, or cannot send an authorization header, `dotpm mcp` bridges stdio to that endpoint and finds the token itself, so no secret sits in the client's config:
